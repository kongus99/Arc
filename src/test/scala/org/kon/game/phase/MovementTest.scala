...
class MovementTest extends FunSpec {
  ...
  describe("A simple adjacent move") {
    ...
    it("is valid when fields are adjacent") {
      assert(move.isValid(investigator, BoardFactory.create((1, 3) ::(3, 1) :: Nil, Nil)))
    }
    it("is not valid when fields are not directly adjacent") {
      assert(move.isValid(investigator, BoardFactory.create((1, 2) ::(2, 1) ::(2, 3) ::(3, 2) :: Nil, Nil)) === false)
    }
    it("is valid when fields are adjacent and there is second neighbor") {
      assert(move.isValid(investigator, BoardFactory.create((1, 3) ::(1, 2) ::(3, 1) ::(2, 1) :: Nil, Nil)))
    }
    it("is valid when there is only reverse adjacency provided") {
      assert(move.isValid(investigator, BoardFactory.create((3, 1) :: Nil, Nil)))
    }
  }
  ...
  describe("A back and forth move") {
    ...
    it("is valid when fields are adjacent") {
      assert(move.isValid(investigator, BoardFactory.create((1, 3) :: Nil, Nil)))
    }
    it("is not valid when fields are not directly adjacent") {
      assert(move.isValid(investigator, BoardFactory.create((1, 2) ::(2, 1) ::(2, 3) ::(3, 2) :: Nil, Nil)) === false)
    }
  ...
  describe("A two field move") {
    ...
    it("is valid when there is a path between fields") {
      assert(move.isValid(investigator, BoardFactory.create((1, 3) ::(2, 3) :: Nil, Nil)))
    }

    it("is not valid when there is a missing second step") {
      assert(move.isValid(investigator, BoardFactory.create((1, 3) :: Nil, Nil)) === false)
    }
  ...
  describe("A three field move") {
    ...
    it("is valid when there is a path between fields") {
      assert(move.isValid(investigator, BoardFactory.create((1, 3) ::(2, 3) ::(4, 2) :: Nil, Nil)))
    }

    it("is not valid when there is a missing first step") {
      assert(move.isValid(investigator, BoardFactory.create((2, 3) ::(4, 2) :: Nil, Nil)) === false)
    }

    it("is not valid when there is a missing second step") {
      assert(move.isValid(investigator, BoardFactory.create((4, 2) ::(1, 3) :: Nil, Nil)) === false)
    }

    it("is not valid when there is a missing third step") {
      assert(move.isValid(investigator, BoardFactory.create((3, 1) ::(3, 2) :: Nil, Nil)) === false)
    }
  ...
  describe("A three field loop move") {
    ...
    it("is valid when there is a path between fields") {
      assert(move.isValid(investigator, BoardFactory.create((1, 3) ::(2, 3) ::(1, 2) :: Nil, Nil)))
    }
  ...
  describe("A move that uses two movement points") {
    ...
    it("should not be valid if player has only one move point") {
      ...
      assert(move.isValid(investigator, BoardFactory.create((1, 3) ::(2, 3) ::(1, 2) :: Nil, Nil)) === false)
    }
  ...
  describe("A two field move through a blocking field") {
    ...
    it("should not be valid if adjacent field is blocked") {
      val investigator: Investigator[Possession] = new Investigator[Possession](1, 2, Nil)
      assert(move.isValid(investigator, BoardFactory.create((1, 3) ::(2, 3) ::(1, 2) :: Nil, 3 :: Nil)) === false)
    }
    it("should not be valid if second field is blocked") {
      val investigator: Investigator[Possession] = new Investigator[Possession](1, 2, Nil)
      assert(move.isValid(investigator, BoardFactory.create((1, 3) ::(2, 3) ::(1, 2) :: Nil, 2 :: Nil)) === false)
    }
  }
